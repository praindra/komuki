...
import * as adminService from '../services/adminService'; // Untuk notifikasi WhatsApp
...
const AdminSchedules = () => {
    ...
    const [editingDoctor, setEditingDoctor] = useState(null); // Menyimpan dokter yang sedang diedit
    ...
    const handleEditDoctor = (doctor) => {
        setEditingDoctor({ ...doctor }); // Salin objek dokter untuk diedit
    };
    ...
    const handleSendVacationNotification = async () => {
        if (!vacationDoctor || !vacationDate) {
            setPopupMessage('Pilih dokter dan tanggal cuti.');
            setPopupType('error');
            setShowPopup(true);
            return;
        }
        const doctor = doctors.find(doc => doc._id === vacationDoctor);
        if (!doctor) {
            setPopupMessage('Dokter tidak ditemukan.');
            setPopupType('error');
            setShowPopup(true);
            return;
        }

        const message = `Halo pasien Klinik Sehat, kami ingin memberitahukan bahwa dr. ${doctor.name} akan cuti pada tanggal ${new Date(vacationDate).toLocaleDateString()}. Mohon maaf atas ketidaknyamanannya.`;
        // Dalam implementasi nyata, Anda akan mengambil nomor telepon pasien yang relevan dari reservasi
        // Misalnya, Anda akan mengambil semua reservasi di tanggal tersebut, lalu mengirimkan ke setiap pasien.
        // Untuk contoh ini, kita akan mengirim ke nomor placeholder.
        const examplePhoneNumber = process.env.WHATSAPP_SENDER_NUMBER; // Ini akan menjadi nomor yang terdaftar di WhatsApp API

        try {
            await adminService.sendWhatsappNotification(doctor._id, vacationDate, message);
            setPopupMessage('Pemberitahuan cuti berhasil dikirim ke pasien yang terkait.');
            setPopupType('success');
            setShowPopup(true);
            setVacationDoctor('');
            setVacationDate('');
            ...
            setPopupMessage(error.response?.data?.msg || 'Gagal mengirim pemberitahuan cuti.');
            setPopupType('error');
            setShowPopup(true);
        }
    };
